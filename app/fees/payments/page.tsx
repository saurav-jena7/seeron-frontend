...
export default function FeePaymentsPage() {
  ...
  const [form, setForm] = useState({
    student_id: '', fee_category_id: '', amount: '',
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await api.get('/fees/payments', {
  ...
  useEffect(() => {
    load();
    api.get('/fees/categories').then(r => setCategories(r.data.data || [])).catch(() => {});
  }, [page, search]);
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <CreditCard className="w-5 h-5 text-indigo-600" /> Fee Payments
              </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Input label="Payment Date *" type="date" value={form.payment_date}
                onChange={e => f('payment_date', e.target.value)} required />
              ...
              <Input label="Transaction Ref" value={form.transaction_ref}
                onChange={e => f('transaction_ref', e.target.value)} placeholder="Optional" />
            </div>
