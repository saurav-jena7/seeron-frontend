...
interface FeeCategory {
  id: string; name: string; description: string; amount: number; frequency: string;
}
...
export default function FeeCategoriesPage() {
  ...
  function openEdit(c: FeeCategory) {
    setEditing(c);
    setForm({ name: c.name, description: c.description || '', amount: String(c.amount), frequency: c.frequency });
    setOpen(true);
  }
  ...
  const freqVariant = (f: string): 'info' | 'success' | 'purple' | 'warning' =>
    f === 'monthly' ? 'info' : f === 'annually' ? 'success' : f === 'one_time' ? 'purple' : 'warning';
    ...
    <AuthGuard anyPermission={['fee.view']}>
      <AppShell title="Fee Categories">
        <div className="space-y-5 max-w-3xl">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              <Tag className="w-5 h-5 text-indigo-600" /> Fee Categories
            </h2>
            ...
            <CardContent className="p-0">
              {loading ? <Spinner /> : (
                <Table>
                  <Thead>
                    <tr>
                      <Th>Name</Th><Th>Description</Th><Th>Amount</Th><Th>Frequency</Th><Th>Actions</Th>
                    </tr>
                  </Thead>
                  <Tbody>
                    {categories.length === 0 ? (
                      ...
                      <Tr key={c.id}>
                        ...
                        <Td>
                          <div className="flex gap-1">
                            <button onClick={() => openEdit(c)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg">
                              <Pencil className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => del(c)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg">
        ...
        <Modal open={open} onClose={() => setOpen(false)}
          title={editing ? 'Edit Category' : 'Add Fee Category'} size="sm">
