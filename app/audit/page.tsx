...
export default function AuditPage() {
  ...
  useEffect(() => {
    api.get('/audit/actions').then((r) => { setActions(r.data.data.actions); setResources(r.data.data.resources); });
  }, []);
  ...
  async function load() {
    ...
    try {
      const r = await api.get('/audit', {
        params: { page, limit, action: filterAction || undefined, resource: filterResource || undefined, from_date: fromDate || undefined, to_date: toDate || undefined }
      });
      setLogs(r.data.data); setTotal(r.data.meta.total);
    } catch {}
    setLoading(false);
  }
    ...
    <AuthGuard anyPermission={['audit.view']}>
      <AppShell title="Activity Log">
        <div className="space-y-5">
          <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2"><Activity className="w-5 h-5 text-indigo-600" /> Activity / Audit Log</h2>
            ...
            <CardHeader>
              <div className="flex flex-wrap gap-3 w-full items-end">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => { setFilterAction(''); setFilterResource(''); setFromDate(''); setToDate(''); setPage(1); }}>Clear</Button>
                </div>
                      ...
                      : logs.map((log) => (
                        <Tr key={log.id}>
                          ...
                          <Td>
                            ...
                            {log.resource_id && <span className="text-xs text-gray-400 ml-1 font-mono">#{log.resource_id.slice(0, 8)}</span>}
                          </Td>
                          <Td>
