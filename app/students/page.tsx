...
export default function StudentsPage() {
  ...
  async function load() {
    ...
    try {
      const r = await api.get('/students', {
        params: { page, limit, search: search || undefined, class_id: filterClass || undefined },
      });
  ...
  // filter sections whose populated class.id matches the selected class in the form
  const filteredSections = sections.filter((s) => s.class?.id === form.class_id);

  function openAdd() {
    setEditing(null);
    setForm({ ...emptyForm, academic_year_id: years.find((y) => y.is_current)?.id || '' });
    setOpen(true);
  }
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <GraduationCap className="w-5 h-5 text-indigo-600" /> Students
              </h2>
              ...
              {loading ? <Spinner /> : (
                <Table>
                  ...
                  <Tbody>
                    {students.length === 0 ? (
                      <Tr>
                        <Td className="text-center text-gray-400 py-8" colSpan={9}>No students found</Td>
                      </Tr>
                    ) : students.map((s) => (
                      <Tr key={s.id}>
                        <Td>
